...
class Question(models.Model):
    ...
    description_question = models.CharField(max_length=50, verbose_name='Краткое описание')
...
class User(AbstractUser):
    name = models.CharField(max_length=200, verbose_name='Имя', blank=False, validators=[
        RegexValidator(
            regex='^[А-Яа-я -]*$',
            message='Разрешается использовать только кириллицу',
    ...
    username = models.CharField(max_length=200, verbose_name='Логин', unique=True, blank=False, validators=[
        RegexValidator(
            regex='^[A-Za-z -]*$',
            message='Разрешается использовать только латиницу',
            code='invalid_username'
        ),
    ])
